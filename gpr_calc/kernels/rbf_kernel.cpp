...
extern "C"
void rbf_kff_many_diag(int n1, int d, int x2i, double zeta, double sigma2, double l2, double tol,
              double* x1, double* dx1dr, int* ele1, int* x1_inds, 
              double* x2, double* dx2dr, int* ele2, int* x2_inds, double* pout){
    double eps=1e-8;
    double dval1, dval2, dval;
    double x1_norm, x2_norm, _x2_norm2, _x1x2_norm, x1x2_dot;
    double _x1x2_norm31, x1x2_dot_norm02, x1x2_dot_norm13, x1x2_dot_norm31;
    double  dx, d2, d1, d2d_dx1dx2;
    double dddx1, dddx2, dDdx1, dDdx2, d2Ddx1dx2;
    ...
    for(ii=0; ii<n1; ii++){
        ...
        dval2=0.0;
    	for(i=0;i<d;i++){
            dval1 += x1[ii*d+i]*x1[ii*d+i];
            dval2 += x2[ii*d+i]*x2[ii*d+i];
    	}
    	x1_norm=sqrt(dval1);
    	x2_norm=sqrt(dval2);

    	if(_ele1==_ele2 && x1_norm > eps && x2_norm > eps){
    	    _x2_norm2 = 1.0/(x2_norm*x2_norm);
            _x1x2_norm = 1.0/(x1_norm*x2_norm);
            _x1x2_norm31 = _x1x2_norm/(x1_norm*x1_norm);
            ...
            if(dK_dD > tol){
                ...
                for(j=0;j<d;j++){
                    C1 += C[j*3+0]*dx2dr[(ii*d+j)*3 + 0];
                    C2 += C[j*3+1]*dx2dr[(ii*d+j)*3 + 0];
                    C3 += C[j*3+2]*dx2dr[(ii*d+j)*3 + 0];
                    C4 += C[j*3+0]*dx2dr[(ii*d+j)*3 + 1];
                    C5 += C[j*3+1]*dx2dr[(ii*d+j)*3 + 1];
                    C6 += C[j*3+2]*dx2dr[(ii*d+j)*3 + 1];
                    C7 += C[j*3+0]*dx2dr[(ii*d+j)*3 + 2];
                    C8 += C[j*3+1]*dx2dr[(ii*d+j)*3 + 2];
                    C9 += C[j*3+2]*dx2dr[(ii*d+j)*3 + 2];
                }
